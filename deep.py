...
def video_mode():
    video_path = input("解析したい動画ファイル名(例:sample.mp4)を入力してください: ")
    ...
    out_pose = cv2.VideoWriter('pose_video_from_file.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
    out_original = cv2.VideoWriter('original_video_from_file.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))

    csv_file = open("multipose_landmarks_from_file.csv", mode='w', newline='')
    writer = csv.writer(csv_file)
    header = ["frame", "timestamp", "person_id"] + [f"{c}{i}" for i in range(33) for c in ("x", "y", "z", "visibility")]
    writer.writerow(header)

    frame_index = 0
    print("解析中: ESCキーで終了します。")
    ...
    while cap.isOpened():
        ...
        out_original.write(original)
        out_pose.write(frame)
        cv2.imshow('Pose Tracking from Video (ESCで終了)', frame)
    ...
    print("解析終了。CSVファイルをperson_idで分割・並び替えます。")
    split_csv_by_person_id("multipose_landmarks_from_file.csv")
    sort_csv_by_person_id("multipose_landmarks_from_file.csv")

    if input("解析した動画を再生しますか？ (y/n): ").lower() == 'y':
        replay_pose_video_from_file()

def replay_pose_video_from_file():
    cap1 = cv2.VideoCapture('original_video_from_file.mp4')
    cap2 = cv2.VideoCapture('pose_video_from_file.mp4')
    ...
    while cap1.isOpened() and cap2.isOpened():
        ...
        cv2.putText(combo, f"Frame: {frame_index}/{total_frames}", (30, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.imshow('Original | Pose (from file)', combo)
